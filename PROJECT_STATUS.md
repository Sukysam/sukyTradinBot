...
**Last updated**: 2026-07-12 · **Current milestone**: 4 — HMM & Regime Detection (not started)
...
| 4 | HMM & Regime Detection | ⏳ Planned | Regime detection model lifecycle, consuming `FeatureVector` only: normalize → train → infer → regime. Persistence and refresh cadence for the fitted model. | Closes Known Gaps item 3 (model store). A reference forward-inference implementation already exists at `regime-trader/core/hmm_engine.py` (`ForwardFilter`, BIC model selection) — this milestone builds the missing persistence/refresh layer and re-points inference at Milestone 3's feature pipeline instead of `data/feature_engineering.py` directly. Depends on Milestone 3. First real consumer of the now-frozen `FeatureVector` contract (v2, with `provenance` — see [ADR-004](docs/engineering-handbook/Architecture/ADR/ADR-004-FeatureVector-Contract-Freeze.md), [ADR-005](docs/engineering-handbook/Architecture/ADR/ADR-005-FeatureVector-Provenance.md), and [Standards/FeatureVector Contract.md](docs/engineering-handbook/Standards/FeatureVector%20Contract.md)); read before wiring the HMM to it. Per user architectural guidance (2026-07-12): once the HMM's real output shape exists, freeze it too as a `RegimeState` contract (candidate `ADR-006` + `Standards/RegimeState Contract.md`) before Milestone 5 (Strategy Engine) becomes a consumer — freeze the interface (fields, model-loading/inference entry points), not implementation details like normalization method, covariance type, hidden-state count, initialization, or convergence thresholds, which are expected to be refined as performance is evaluated. |
