...
**Last updated**: 2026-07-12 · **Current milestone**: 5 — Strategy Engine (contract frozen, implementation not started)
...
| 5 | Strategy Engine | 🚧 In progress | Regime-tier → allocation logic: what a given regime + feature context implies about position bias and sizing, independent of arbitrating it against other signal sources. | Closes the `core/regime_strategies.py` half of Known Gaps item 4 (or an earlier phase toward it — see Master Charter's Capability Ownership Map note). Depends on Milestone 4. Feeds Milestone 11, doesn't merge signals itself — see that milestone for why the two were split. First real consumer of the now-frozen `RegimeState` contract — see [ADR-006](docs/engineering-handbook/Architecture/ADR/ADR-006-RegimeState-Contract.md), [ADR-007](docs/engineering-handbook/Architecture/ADR/ADR-007-HMM-Design.md), and [Standards/RegimeState Contract.md](docs/engineering-handbook/Standards/RegimeState%20Contract.md); read before consuming `hmm.service.RegimeService`. Also the milestone that should resolve the `main.py.ModelStore` Protocol tension noted in ADR-007 Decision 7. **`StrategyDecision` is now frozen ahead of implementation** — [ADR-008](docs/engineering-handbook/Architecture/ADR/ADR-008-StrategyDecision-Contract.md), binding spec: [Standards/StrategyDecision Contract.md](docs/engineering-handbook/Standards/StrategyDecision%20Contract.md). Per plan: `src/strategy/` (registry.py, interfaces.py, models.py, allocator.py, service.py, exceptions.py, strategies/{bull,bear,sideways,defensive}.py) is scoped to regime→allocation only — no capital/liquidity/leverage checks, no order placement, no broker/risk/memory/NLP integration (those are Milestones 6/7/9/10). Add a `RegimeState`→`StrategyDecision` latency benchmark alongside the existing history once implemented. Implementation begins only after this contract freeze is reviewed. |
