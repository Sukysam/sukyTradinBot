...
**Last updated**: 2026-07-13 · **Current milestone**: 6 — Risk Management (contract frozen, implementation not started)
...
| 6 | Risk Management | 🚧 In progress | Converts a `StrategyDecision` (plus a portfolio/account snapshot) into an `ExecutionDecision`: is this trade allowed, and at what size — exposure/concentration limits, leverage, circuit breakers, emergency hard stop. Not order placement. | A complete reference implementation already exists at `regime-trader/core/risk_manager.py` (see Capability Ownership Map). This milestone packages/hardens it under `src/risk/`, not a from-scratch build — grounded in that module's real `VetoDecision`/`CircuitBreakerDecision` shapes and [Standards/Risk Limits Reference.md](docs/engineering-handbook/Standards/Risk%20Limits%20Reference.md)'s actual limits. **`ExecutionDecision` is now frozen ahead of implementation** — [ADR-010](docs/engineering-handbook/Architecture/ADR/ADR-010-ExecutionDecision-Contract.md), binding spec: [Standards/ExecutionDecision Contract.md](docs/engineering-handbook/Standards/ExecutionDecision%20Contract.md). `approved_allocation` is bounded by `strategy_reference.allocation` (risk only ever reduces size), and a size-cut approval must always carry a `risk_adjustments` reason — a deliberate improvement over the legacy `VetoDecision`, which drops that reason today. Per plan: `src/risk/` (config.py, interfaces.py, models.py, validators.py, sizing.py, limits.py, circuit_breakers.py, service.py, exceptions.py) is scoped to evaluation only — no broker calls, no order construction (Milestone 7). Add a `StrategyDecision`→`ExecutionDecision` latency benchmark alongside the existing history once implemented. Implementation begins only after this contract freeze is reviewed. |
