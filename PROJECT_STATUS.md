...
**Last updated**: 2026-07-13 · **Current milestone**: 9 — Adaptive Learning / Memory Loop (contract frozen, implementation not started)
...
| 9 | Adaptive Learning | 🚧 In progress (contract freeze) | Shadow-mode-only memory loop: a contextual bandit (Thompson Sampling over Beta posteriors, `(strategy_id, regime_id)` context) that records what it *would* have recommended for every real `StrategyDecision`, without ever influencing `strategy`/`risk`/`execution`. Adapts the existing Thompson-sampling design rather than porting it verbatim — narrower context, frozen contract, no live wiring in this milestone. | `ExperienceRecord`/`LearningDecision` frozen ahead of implementation — [ADR-016](docs/engineering-handbook/Architecture/ADR/ADR-016-LearningDecision-Contract.md) (contract); binding spec: [Standards/LearningDecision Contract.md](docs/engineering-handbook/Standards/LearningDecision%20Contract.md). Reference implementation for the underlying bandit formulation exists at `regime-trader/core/learning_engine.py` — see [Architecture/Reinforcement Learning Memory Loop.md](docs/engineering-handbook/Architecture/Reinforcement%20Learning%20Memory%20Loop.md); left untouched by this freeze. SHAP-based `rationale` and a LightGBM policy are both explicitly deferred — see ADR-016's Alternatives Considered. Letting a `LearningDecision` actually influence production is a separate, later, explicitly-authorized decision, not assumed here. |
