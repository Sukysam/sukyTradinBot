...
| Health & Readiness (M12 WP1) | `ops` | 0.1.0 | — (deliberately not a frozen domain contract; `PlatformHealth`/`HealthCheckResult` are a documented, stable operational model instead — see ADR-022) | — | none — a leaf package; reads no other first-party package's contract and nothing yet consumes it | [ADR-022](engineering-handbook/Architecture/ADR/ADR-022-Health-And-Readiness-Design.md) (design + implementation, no separate freeze) |
