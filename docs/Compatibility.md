...
| Health & Readiness / Observability (M12 WP1-WP2) | `ops` | 0.2.0 | — (deliberately not a frozen domain contract; `PlatformHealth`/`HealthCheckResult`/`PlatformInfo` are a documented, stable operational model instead — see ADR-022, ADR-023) | — | none — a leaf package; reads no other first-party package's contract and nothing yet consumes it | [ADR-022](engineering-handbook/Architecture/ADR/ADR-022-Health-And-Readiness-Design.md) (WP1: design + implementation, no separate freeze), [ADR-023](engineering-handbook/Architecture/ADR/ADR-023-Observability-Design.md) (WP2: design + implementation, no separate freeze) |
