...
| Health & Readiness / Observability / Configuration & Secrets (M12 WP1-WP3) | `ops` | 0.3.0 | — (deliberately not a frozen domain contract; `PlatformHealth`/`HealthCheckResult`/`PlatformInfo`/`RuntimeContext` are a documented, stable operational model instead — see ADR-022, ADR-023, ADR-024) | — | none — a leaf package; reads no other first-party package's contract and nothing yet consumes it | [ADR-022](engineering-handbook/Architecture/ADR/ADR-022-Health-And-Readiness-Design.md) (WP1), [ADR-023](engineering-handbook/Architecture/ADR/ADR-023-Observability-Design.md) (WP2), [ADR-024](engineering-handbook/Architecture/ADR/ADR-024-Configuration-And-Secrets-Design.md) (WP3) — all design + implementation, no separate freeze |
