...
| Adaptive Learning / Memory Loop | `memory` | 0.1.0 | `ExperienceRecord`, `LearningDecision` | v1 | `orchestration` — advisory only, consumed as `LearningDecision` alongside a `StrategyDecision`; no consumer permitted to act on it in isolation | [ADR-016](engineering-handbook/Architecture/ADR/ADR-016-LearningDecision-Contract.md) (contract freeze), [ADR-017](engineering-handbook/Architecture/ADR/ADR-017-Memory-Loop-Design.md) (design); binding spec: [Standards/LearningDecision Contract.md](engineering-handbook/Standards/LearningDecision%20Contract.md) |
| NLP & Event Processing | `nlp` | 0.1.0 | `NewsSignal` | v1 | `orchestration` — advisory only, consumed alongside a `StrategyDecision`; no consumer permitted to act on it in isolation | [ADR-018](engineering-handbook/Architecture/ADR/ADR-018-NewsSignal-Contract.md) (contract freeze), [ADR-019](engineering-handbook/Architecture/ADR/ADR-019-NLP-News-Engine-Design.md) (design); binding spec: [Standards/NewsSignal Contract.md](engineering-handbook/Standards/NewsSignal%20Contract.md) |
| Signal Orchestration | `orchestration` | 0.1.0 | `FinalDecision`, `SignalInput` | v1 | none yet — wiring `FinalDecision` into `risk.RiskService` is explicitly not authorized by this freeze | [ADR-020](engineering-handbook/Architecture/ADR/ADR-020-FinalDecision-Contract.md) (contract freeze), [ADR-021](engineering-handbook/Architecture/ADR/ADR-021-Signal-Orchestration-Design.md) (design); binding spec: [Standards/FinalDecision Contract.md](engineering-handbook/Standards/FinalDecision%20Contract.md) |
| Health & Readiness / Observability / Configuration & Secrets / Deployment / Operations (M12 WP1-WP5) | `ops` | 0.5.0 | — (deliberately not a frozen domain contract; `PlatformHealth`/`HealthCheckResult`/`PlatformInfo`/`RuntimeContext`/`DeploymentInfo`/`DiagnosticReport` are a documented, stable operational model instead — see ADR-022 through ADR-026) | — | none — a leaf package; reads no other first-party package's contract and nothing yet consumes it | [ADR-022](engineering-handbook/Architecture/ADR/ADR-022-Health-And-Readiness-Design.md) (WP1), [ADR-023](engineering-handbook/Architecture/ADR/ADR-023-Observability-Design.md) (WP2), [ADR-024](engineering-handbook/Architecture/ADR/ADR-024-Configuration-And-Secrets-Design.md) (WP3), [ADR-025](engineering-handbook/Architecture/ADR/ADR-025-Deployment-And-Release-Automation-Design.md) (WP4), [ADR-026](engineering-handbook/Architecture/ADR/ADR-026-Operations-And-Diagnostics-Design.md) (WP5) — all design + implementation, no separate freeze |
