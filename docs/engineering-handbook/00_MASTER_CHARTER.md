...
| Reinforcement Learning Memory Loop (legacy, live-influencing) | **Implemented** (contextual multi-armed bandit) in `regime-trader/`; see the `src/memory/` row below for its Milestone 9 successor, whose contract is frozen but which remains shadow-mode-only and does not touch this legacy path | [05 Memory Engineer](05_MEMORY_ENGINEER.md) | `core/learning_engine.py`, `data/trade_context_db.json`, `data/learning_weights.json` |
| Online Learning | **Implemented** (weekly incremental posterior updates) in `regime-trader/` | [05 Memory Engineer](05_MEMORY_ENGINEER.md) / [04 Quant Researcher](04_QUANT_RESEARCHER.md) | `learning_engine.run_weekly_optimization`; HMM refresh cadence is **planned** |
...
| FinBERT NLP News Engine (legacy) | **Implemented** in `regime-trader/`; see the `src/nlp/` row below for its Milestone 10 successor, whose contract is frozen but which remains shadow-mode-only and does not touch this legacy path | [06 NLP Engineer](06_NLP_ENGINEER.md) | `core/sentiment_engine.py`, `broker/news_streamer.py` |
...
| Backtesting & Validation Platform (`src/backtest/`) | **Implemented** (`replay.run_replay` deterministic full-pipeline replay, `PortfolioEngine`, grouped `metrics/` subpackage, `BacktestEngine`, golden-dataset regression baseline); `BacktestResult` contract frozen at v1. Replays historical bars through the entire decision pipeline (Market Data → Features → HMM → Strategy → Risk → Execution), simulates fills at each bar's own open, and computes performance metrics — never retrains models | [04 Quant Researcher](04_QUANT_RESEARCHER.md) (metric correctness, replay fidelity) / [09 QA Engineer](09_QA_ENGINEER.md) (golden-dataset/regression methodology) | `src/backtest/` — see [ADR-014](Architecture/ADR/ADR-014-BacktestResult-Contract.md) (contract freeze) and [ADR-015](Architecture/ADR/ADR-015-Backtesting-Engine-Design.md) (design); binding spec: [Standards/BacktestResult Contract.md](Standards/BacktestResult%20Contract.md). Distinct from the pre-existing `backtest/` crypto sandbox above — a new, higher-stakes build under this repository's own tooling, not an extension of it. Only exercised against deterministic synthetic data (`SYNTH`, never real historical prices — no live market-data credentials exist, see Known Gaps). |
...
| Adaptive Learning / Memory Loop Platform (`src/memory/`) | **Implemented** (`InMemoryExperienceStore`/`JsonlExperienceStore` append-only Experience Store, `ThompsonSamplingPolicy`/`BetaArm` contextual bandit, `MemoryService`, `evaluation.evaluate`/`generate_evaluation_report`); `ExperienceRecord`/`LearningDecision` contracts frozen at v1. Adapts the legacy contextual-bandit formulation (Thompson Sampling over Beta posteriors, `(strategy_id, regime_id)` context) behind two frozen types: `ExperienceRecord` (closed-trade outcome, the Experience Store's atomic unit) and `LearningDecision` (a shadow allocation recommendation, `[0.0, 1.0]`, compared against but never substituted for the real `StrategyDecision.allocation`). SHAP-based rationale and a LightGBM policy are both explicitly deferred in favor of a simple posterior summary and the same bandit the legacy code already validated | [05 Memory Engineer](05_MEMORY_ENGINEER.md) | `src/memory/` — see [ADR-016](Architecture/ADR/ADR-016-LearningDecision-Contract.md) (contract freeze) and [ADR-017](Architecture/ADR/ADR-017-Memory-Loop-Design.md) (design); binding spec: [Standards/LearningDecision Contract.md](Standards/LearningDecision%20Contract.md). **Shadow mode only**: no consumer in this milestone may act on `LearningDecision.recommended_allocation` — `strategy`, `risk`, and `execution` gain no new dependency on this package. Only exercised against synthetic `ExperienceRecord`s; not yet wired to a real backtest replay or live trading loop as a producer. |
| NLP & Event Processing Platform (`src/nlp/`) | **Implemented** (`InMemoryNewsItemStore`/`JsonlNewsItemStore` deduplicating on `(source, source_id)`, batch-only `SentimentScorer` Protocol, `DeterministicSentimentScorer` + `FinBertSentimentScorer`, `NlpService`, `evaluation.evaluate_ingestion`/`evaluate_sentiment`/`generate_evaluation_report`); `NewsSignal` contract frozen at v1. Adapts the legacy FinBERT sentiment shape (`SentimentScore`: three-probability-plus-label, `[0.99, 1.01]` sum tolerance) and news-ingestion shape (`NewsItem`) behind one frozen type: `NewsSignal` (headline, sentiment probabilities/label with a type-level argmax-consistency check, extracted entities, traceability fields). Phased: ingestion/dedup/storage first, sentiment scoring second, evaluation third — infrastructure before the model. Entity extraction deferred (`entities=()` always in this milestone) | [06 NLP Engineer](06_NLP_ENGINEER.md) | `src/nlp/` — see [ADR-018](Architecture/ADR/ADR-018-NewsSignal-Contract.md) (contract freeze) and [ADR-019](Architecture/ADR/ADR-019-NLP-News-Engine-Design.md) (design); binding spec: [Standards/NewsSignal Contract.md](Standards/NewsSignal%20Contract.md). **Shadow mode only**: no consumer in this milestone may act on `NewsSignal` — `strategy`, `risk`, and `execution` gain no new dependency on this package. `FinBertSentimentScorer` requires `torch`/`transformers` (the `trading` extra, not installed by base CI) — its own tests are `@pytest.mark.integration`-gated and skip gracefully via `pytest.importorskip` when absent. |
| Signal Orchestration Platform (`src/orchestration/`) | **Implemented** (`arbitrate` + four `ArbitrationPolicy` implementations — `SafetyFirstPolicy` default, `ConsensusPolicy`, `WeightedVotePolicy`, `ConfidencePolicy` — `orchestration.signals` shared agreement logic, `orchestration.evaluate`/`generate_evaluation_report` cross-signal reporting); `FinalDecision`/`SignalInput` contracts frozen at v1. Reconciles `StrategyDecision` (primary), `LearningDecision`, and `NewsSignal` (both advisory) rather than producing an independent opinion. `final_allocation` is type-level bounded to `[0.0, primary_allocation]` — no arbitration policy can let an advisory signal manufacture conviction the Strategy Engine never had. Arbitration algorithm is policy-based, pluggable behind one Protocol | [07 Signal Orchestrator](07_SIGNAL_ORCHESTRATOR.md) | `src/orchestration/` — see [ADR-020](Architecture/ADR/ADR-020-FinalDecision-Contract.md) (contract freeze) and [ADR-021](Architecture/ADR/ADR-021-Signal-Orchestration-Design.md) (design); binding spec: [Standards/FinalDecision Contract.md](Standards/FinalDecision%20Contract.md). **Wiring not yet authorized**: this milestone does not permit replacing `StrategyDecision` with `FinalDecision` as `risk.RiskService`'s input — `risk` gains no new dependency on this package. |
| Production Deployment / Operational Maturity (`src/ops/`) | **Implemented** (process lifecycle; WP1 Health & Readiness — `PlatformHealth`/`HealthCheckResult` operational model, `classify_status` single-source-of-truth aggregation, ten named `HealthCheck` subsystem factories behind one generic `CallableHealthCheck`, `evaluate_health`/`require_healthy` fail-fast startup gate; WP2 Observability — `PlatformInfo` build identity, `ops.metrics` `Counter`/`Gauge`/`MetricsRegistry` + `record_health_metrics` + Prometheus-text export, `ops.tracing` hook-based `Span`/`Tracer`, `ops.logging` structured operational events, `ops.alerts` `CallableAlertRule` + named rule factories + `evaluate_alerts` — every WP2 module reads `PlatformHealth` as the single operational model, none recomputes health independently; WP3 Configuration & Secrets — `ops.secrets` injectable `SecretSource`/`EnvSecretSource`/`SecretValue` (redacted repr), `ops.validation` fail-fast `validate_runtime`/`require_valid_runtime` mirroring `ops.health`'s report/gate split, `ops.models.RuntimeContext` immutable runtime identity, `ops.startup.build_runtime_context` the one startup composition entrypoint; WP4 Deployment & Release Automation — `ops.models.DeploymentInfo` one deployment instance distinct from `PlatformInfo`, `ops.deployment.validate_deployment`/`ReleaseManifest`/`verify_release_manifest` (reusing `ops.validation.ValidationResult`), `ops.rollback.select_rollback_target` last-known-good selection over deployment history — literal deployment manifests/CI "deploy" jobs/release scripts deliberately deferred, no deployment target chosen yet; WP5 Operations & Runbooks — `ops.models.DiagnosticReport` composing WP1-WP4's models without duplication, `ops.diagnostics.build_diagnostic_report`/`ops.reporting.generate_diagnostic_report`, six `docs/operations/` runbooks (release, incident response, disaster recovery, backup/restore, on-call, production checklist) complementing the pre-existing SOPs. **Milestone 12 feature-complete across all five work packages as of WP5.**) | [12 DevOps Engineer](12_DEVOPS_ENGINEER.md) | `main.py` lifecycle; `src/ops/` — see [ADR-022](Architecture/ADR/ADR-022-Health-And-Readiness-Design.md) (WP1), [ADR-023](Architecture/ADR/ADR-023-Observability-Design.md) (WP2), [ADR-024](Architecture/ADR/ADR-024-Configuration-And-Secrets-Design.md) (WP3), [ADR-025](Architecture/ADR/ADR-025-Deployment-And-Release-Automation-Design.md) (WP4), and [ADR-026](Architecture/ADR/ADR-026-Operations-And-Diagnostics-Design.md) (WP5) — all design + implementation, no separate freeze — operational maturity work, not a domain-decision milestone. See also [Architecture/Production Deployment.md](Architecture/Production%20Deployment.md) and `docs/operations/`. |
