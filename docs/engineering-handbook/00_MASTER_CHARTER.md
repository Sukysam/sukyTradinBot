...
| Signal Orchestration Platform (`src/orchestration/`) | **Contract frozen**, implementation not started (Milestone 11). The first milestone whose purpose is reconciling `StrategyDecision` (primary), `LearningDecision`, and `NewsSignal` (both advisory) rather than producing an independent opinion. `final_allocation` is type-level bounded to `[0.0, primary_allocation]` — no arbitration policy can let an advisory signal manufacture conviction the Strategy Engine never had. Arbitration algorithm (weighted-vote, confidence-threshold, consensus, safety-first) is deliberately unfrozen, policy-based implementation detail | [07 Signal Orchestrator](07_SIGNAL_ORCHESTRATOR.md) | `src/orchestration/` (not yet scaffolded) — see [ADR-020](Architecture/ADR/ADR-020-FinalDecision-Contract.md) (contract freeze); binding spec: [Standards/FinalDecision Contract.md](Standards/FinalDecision%20Contract.md). **Wiring not yet authorized**: this freeze does not permit replacing `StrategyDecision` with `FinalDecision` as `risk.RiskService`'s input — `risk` gains no new dependency on this package in this milestone. |
