...
| NLP & Event Processing Platform (`src/nlp/`) | **Contract frozen**, implementation not started (Milestone 10). Adapts the legacy FinBERT sentiment shape (`SentimentScore`: three-probability-plus-label, `[0.99, 1.01]` sum tolerance) and news-ingestion shape (`NewsItem`) behind one frozen type: `NewsSignal` (headline, sentiment probabilities/label with a new type-level argmax-consistency check, extracted entities, traceability fields). Phased: ingestion/dedup/storage first, FinBERT sentiment scoring second, SHAP attribution third — infrastructure before the model | [06 NLP Engineer](06_NLP_ENGINEER.md) | `src/nlp/` (not yet scaffolded) — see [ADR-018](Architecture/ADR/ADR-018-NewsSignal-Contract.md) (contract freeze); binding spec: [Standards/NewsSignal Contract.md](Standards/NewsSignal%20Contract.md). **Shadow mode only**: no consumer in this milestone may act on `NewsSignal` — `strategy`, `risk`, and `execution` gain no new dependency on this package. |
