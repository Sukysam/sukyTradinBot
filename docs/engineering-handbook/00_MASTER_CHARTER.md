...
| Backtesting & Validation Platform (`src/backtest/`) | **Contract frozen** (`BacktestResult` v1); no implementation yet. Replays historical bars through the entire decision pipeline (Market Data → Features → HMM → Strategy → Risk → Execution), simulates fills, and computes performance metrics — never retrains models | [04 Quant Researcher](04_QUANT_RESEARCHER.md) (metric correctness, replay fidelity) / [09 QA Engineer](09_QA_ENGINEER.md) (golden-dataset/regression methodology) | Not yet built — see [ADR-014](Architecture/ADR/ADR-014-BacktestResult-Contract.md); binding spec: [Standards/BacktestResult Contract.md](Standards/BacktestResult%20Contract.md). Distinct from the pre-existing `backtest/` crypto sandbox above — a new, higher-stakes build under this repository's own tooling, not an extension of it. |
