...
def _live_vector(value: tuple[float, float] = (0.0, 0.0)) -> FeatureVector:
    X = np.array([value])
    return make_feature_vectors(X, FEATURE_NAMES, symbol=SYMBOL)[0]
...
class TestRegimeEmitter:
    def test_emits_regime_state_and_logs_structured_event(
        ...
        with caplog.at_level(logging.INFO, logger="app.regime_loop"):
            emitter.handle_feature_vector(_live_vector())
    ...
    def test_on_regime_state_callback_is_called_with_the_computed_state(self) -> None:
        received: list[RegimeState] = []
        emitter = RegimeEmitter(_trained_service(), on_regime_state=received.append)
    ...
    def test_on_regime_state_callback_failure_is_logged_and_does_not_raise(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
        def _boom(_state: RegimeState) -> None:
            raise RuntimeError("simulated callback failure")

        emitter = RegimeEmitter(_trained_service(), on_regime_state=_boom)

        with caplog.at_level(logging.WARNING, logger="app.regime_loop"):
            emitter.handle_feature_vector(_live_vector())  # must not raise

        failures = [
            r
            for r in caplog.records
            if getattr(r, "event", None) == "on_regime_state_callback_failed"
        ]
